...
import { 
  ...
  User, UserCheck, UploadCloud, CalendarDays
} from "lucide-react";

// ★ 作った部品をインポート！（これだけで機能が全部入ります）
import LocalPdfEditor from "@/components/pdf/LocalPdfEditor";

// --- 以下、メイン画面用のデータ・部品 (ドラムロール等はそのまま維持) ---

const staffList = [
  { id: 's1', name: '田中 太郎 (第1課)', clients: [{ id: 'c1', name: '株式会社 鈴木商店', fiscal: 3, groupId: 'yamada', role: 'main' }, { id: 'c1_sub1', name: '鈴木 太郎 (個人)', fiscal: 3, groupId: 'yamada', role: 'sub' }, { id: 'c2', name: '合同会社 テック', fiscal: 12 }, { id: 'c3', name: '山田建設', fiscal: 9 }] },
  { id: 's2', name: '佐藤 花子 (第2課)', clients: [{ id: 'c4', name: '医療法人 佐藤医院', fiscal: 12, groupId: null, role: 'main' }, { id: 'c5', name: 'クリエイティブ合同', fiscal: 6, groupId: null, role: 'main' }] }
];
const periods = { year: ['R7', 'R6', 'R5', 'R4'], month: Array.from({length: 12}, (_, i) => `${12 - i}月`) };

// ジェスチャー制御 (そのまま維持)
function useCrossSwipe(ref: React.RefObject<HTMLDivElement>, axis: 'h' | 'v', callback: (delta: number) => void) {
  useEffect(() => {
    const element = ref.current;
    if (!element) return;
    let accumulatedDelta = 0; const threshold = 40; let isCoolingDown = false;
    const handleWheel = (e: WheelEvent) => {
      if (isCoolingDown) return;
      if (axis === 'v') {
        if (Math.abs(e.deltaX) > Math.abs(e.deltaY) && Math.abs(e.deltaX) > 10) {
          e.preventDefault(); accumulatedDelta += e.deltaX;
          if (Math.abs(accumulatedDelta) > threshold) { callback(accumulatedDelta > 0 ? 1 : -1); accumulatedDelta = 0; isCoolingDown = true; setTimeout(() => isCoolingDown = false, 500); }
        } else { accumulatedDelta = 0; }
      } else {
        if (Math.abs(e.deltaY) > Math.abs(e.deltaX) && Math.abs(e.deltaY) > 10) {
          e.preventDefault(); accumulatedDelta += e.deltaY;
          if (Math.abs(accumulatedDelta) > threshold) { callback(accumulatedDelta > 0 ? 1 : -1); accumulatedDelta = 0; isCoolingDown = true; setTimeout(() => isCoolingDown = false, 500); }
        } else { accumulatedDelta = 0; }
      }
    };
...
const DropzoneCard = ({ title, onDropFile }: { title: string, onDropFile: (f: File, t: string) => void }) => {
  const onDrop = useCallback((acceptedFiles: File[]) => { if(acceptedFiles[0]) onDropFile(acceptedFiles[0], title); }, [title, onDropFile]);
  const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop, accept: {'application/pdf': ['.pdf']}, noClick: false });
  return (
    <div {...getRootProps()} className={clsx("h-32 rounded-xl border-2 border-dashed flex flex-col items-center justify-center p-2 transition-all cursor-pointer group relative overflow-hidden", isDragActive ? "bg-blue-50 border-blue-500 scale-105 shadow-xl z-10" : "bg-slate-50 border-slate-300 hover:bg-white hover:border-blue-400 hover:scale-105 hover:shadow-md")}>
       <input {...getInputProps()} />
       {isDragActive && <div className="absolute inset-0 bg-blue-100/50 flex items-center justify-center z-20 pointer-events-none"><UploadCloud className="text-blue-600 animate-bounce" size={40} /></div>}
...
export default function Home() {
  ...
  const [uploadedStatus, setUploadedStatus] = useState<Record<string, boolean>>({'決算報告書': true});
  const [toast, setToast] = useState<{show: boolean, icon: any, title: string, msg: string}>({ show: false, icon: null, title: "", msg: "" });
  ...
  const handleUpload = useCallback(async (file: File, docTitle: string) => {
    ...
    setActiveFile(file);
    setViewerOpen(true);
    setUploadedStatus(prev => ({ ...prev, [docTitle]: true }));
    showToast(UploadCloud, "READY TO EDIT", `${docTitle} を読み込みました`);
  }, [showToast]);
  ...
  const docList = isPerm ? ['定款', '履歴事項全部証明書', '株主名簿', '設立届出書'] : (mode === 'year' ? ['決算報告書', '総勘定元帳', '法人税申告書', '消費税申告書'] : ['月次試算表', '通帳コピー', '請求書綴り', '給与台帳']);
  ...
  return (
    <div className="text-slate-600 h-screen flex flex-col bg-slate-100 font-sans overflow-hidden select-none">
           ...
           <div className="flex-1 overflow-y-auto p-8">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 animate-fade-in-up">
                 {docList.map((title) => {
                   ...
                   return isFilled ? (
                     <div key={title} onClick={() => alert("編集するには未提出状態に戻してください")} className={clsx("h-32 rounded-xl p-4 flex flex-col justify-between hover:scale-105 transition-transform cursor-pointer bg-white border-l-4 shadow-sm", isPerm ? "border-yellow-400" : "border-green-500")}>
                        <div className="flex justify-between items-start"><FileText className={clsx("text-xl", isPerm ? "text-yellow-500" : "text-green-500")} /><CheckCircle className="text-green-500" size={20} /></div>
                        <div className="text-sm font-bold text-slate-700 leading-tight">{title}</div>
                        <div className="text-[10px] text-green-600 font-bold bg-green-50 w-fit px-2 rounded">提出済</div>
                     </div>
                   ) : (
      ...
      {/* === ★ 呼び出し元がこんなにシンプルになりました！ === */}
      {viewerOpen && activeFile && (
         <LocalPdfEditor file={activeFile} onClose={() => setViewerOpen(false)} />
      )}
    </div>
  );
}